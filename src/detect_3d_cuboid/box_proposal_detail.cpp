...
void detect_3d_cuboid::detect_cuboid(const cv::Mat &rgb_img, const Matrix4d &transToWolrd, const MatrixXd &obj_bbox_coors,
									 MatrixXd all_lines_raw, std::vector<ObjectSet> &all_object_cuboids, std::string file_num)
{
	...
	double shorted_edge_thre = 1;		// if box edge are too short. box might be too thin. most possibly wrong.
	...
	Vector4d ground_plane_world;
	ground_plane_world << 0, 0, 1, 0; // treated as column vector % in my pop-up code, I use [0 0 -1 0]. here I want the normal pointing innerwards, towards the camera to match surface normal prediction
	...
	for (int object_id = 0; object_id < num_2d_objs; object_id++)
	{
		...
		int center_x_raw = obj_bbox_coors(object_id, 0);
		int center_y_raw = obj_bbox_coors(object_id, 1);
		int obj_width_raw = obj_bbox_coors(object_id, 2);
		int obj_height_raw = obj_bbox_coors(object_id, 3);
		
		// yolo type modify == center --> corners coordinate

		int left_x_raw = center_x_raw - (obj_width_raw / 2);
		int top_y_raw = center_y_raw - (obj_height_raw / 2);
		int right_x_raw = center_x_raw + (obj_width_raw / 2);
		int down_y_raw = center_y_raw + (obj_height_raw / 2);
	...
	if (whether_plot_final_images || whether_save_final_images)
	{
		...
		if (whether_plot_final_images)
		{
			...
			cv::imwrite("/home/cadit/src/copy/RGBD_SLAM/DATA_FOLDER/keyframe_set_annotated/obj_train_data/frame_all_cubes_img"+file_num+".png", frame_all_cubes_img);
			//cv::waitKey(0);
		}
	}
}
